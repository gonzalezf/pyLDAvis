...
var LDAvis = function (to_select, data_or_file_name, tweets) {
    ...
    function visualize(data) {
        ...
        d3.select("#" + topicUp)
            .on("click", function () {
                ...
                show_most_relevant_tweets(value_new)
                state_save(true);
            });
        ...
        d3.select("#" + topicID)
            .on("keyup", function () {
                ...
                if (!isNaN(value_new) && value_new > 0) {
                    ...
                    //alert(document.getElementById(topicID).value)

                    show_most_relevant_tweets(document.getElementById(topicID).value)
                }
            });
        ...
        function show_most_relevant_tweets(topic_id) {
            console.log(tweets);
            var topicTweets = tweets.filter(function (el) {
                return el.Topic_Num == topic_id - 1
            });
            var columns = ["Topic_Perc_Contrib", "text"];
            var tbody = d3.select("#tweets").select("tbody");
